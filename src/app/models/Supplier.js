...
class SupplierModels {
  async index() {
    const query = "SELECT * FROM supplier ORDER BY id";
  ...
  async show(id) {
    const query = "SELECT * FROM supplier WHERE id = $1";
    const conn = await connection.conn();
  ...
  async create(supplier) {
    const query = "INSERT INTO supplier (name) VALUES ($1)";
    const { name } = supplier;
    const conn = await connection.conn();
  ...
  async update(id, supplier) {
    const query = "UPDATE supplier SET name = $1 WHERE id = $2 ";
    const { name } = supplier;
    const conn = await connection.conn();
