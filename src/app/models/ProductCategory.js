...
class ProductCategoriesModels {
  ...
  async create(productCategory) {
    const query = "INSERT INTO product_category (description) VALUES ($1)";
  ...
  async update(id, productCategory) {
    const query = "UPDATE product_category SET description = $1 WHERE id = $2";
    const { description } = productCategory;
    const conn = await connection.conn();
...
export default new ProductCategoriesModels();
